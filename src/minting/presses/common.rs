...
pub fn comp_name_type(data: Arc<UnitArchipelago>) -> Result<String, Error> {
    Ok(format!(
        "{}{}",
        data.lexical_unit_lagoon.general_comp0.unit_pool
            [prelude::random_index(data.lexical_unit_lagoon.general_comp0.unit_pool.len())?],
        data.lexical_unit_lagoon.general_comp1.unit_pool
            [prelude::random_index(data.lexical_unit_lagoon.general_comp1.unit_pool.len())?]
    ))
}
...
pub fn formal_name_die(data: Arc<UnitArchipelago>) -> Result<String, Error> {
    Ok(format!(
        "{} {}",
        data.clone().lexical_unit_lagoon.people_title.unit_pool
            [prelude::random_index(data.lexical_unit_lagoon.people_title.unit_pool.len())?],
        legal_name_die(data)?
    ))
}
...
pub fn simple_name_type(data: Arc<UnitArchipelago>) -> Result<String, Error> {
    ...
    if seed >= 2 {
        Ok(data.lexical_unit_lagoon.place_single.unit_pool
            [prelude::random_index(data.lexical_unit_lagoon.place_single.unit_pool.len())?]
        .clone())
    } else if seed == 1 {
        Ok(data.lexical_unit_lagoon.people_nickname.unit_pool
            [prelude::random_index(data.lexical_unit_lagoon.people_nickname.unit_pool.len())?]
        .clone())
    } else {
        comp_name_type(data)
    }
...
pub fn nickname_type(data: Arc<UnitArchipelago>) -> Result<String, Error> {
    Ok((if prelude::random_from_range(0, 2)? == 0 {
        data.lexical_unit_lagoon.people_nickname.unit_pool
            [prelude::random_index(data.lexical_unit_lagoon.people_nickname.unit_pool.len())?]
        .clone()
    } else {
        String::default()
...
pub fn first_name_type(data: Arc<UnitArchipelago>) -> Result<String, Error> {
    if prelude::random_from_range(0, 1)? == 0 {
        ...
        Ok(data.lexical_unit_lagoon.people_first_name.unit_pool
            [prelude::random_index(data.lexical_unit_lagoon.people_first_name.unit_pool.len())?]
        .clone())
    }
}
...
pub fn last_name_type(data: Arc<UnitArchipelago>) -> Result<String, Error> {
    if prelude::random_from_range(0, 1)? == 0 {
        ...
        Ok(data.lexical_unit_lagoon.people_last_name.unit_pool
            [prelude::random_index(data.lexical_unit_lagoon.people_last_name.unit_pool.len())?]
        .clone())
    }
}
...
pub fn skill_die(data: Arc<UnitArchipelago>) -> Result<String, Error> {
    Ok(format!(
        "{} {}",
        data.lexical_unit_lagoon.people_skill_level.unit_pool
            [prelude::random_index(data.lexical_unit_lagoon.people_skill_level.unit_pool.len())?],
        data.lexical_unit_lagoon.people_skill.unit_pool
            [prelude::random_index(data.lexical_unit_lagoon.people_skill.unit_pool.len())?]
    ))
}

pub fn material_type(data: Arc<UnitArchipelago>) -> Result<String, Error> {
    Ok(data.lexical_unit_lagoon.artifact_material.unit_pool
        [prelude::random_index(data.lexical_unit_lagoon.artifact_material.unit_pool.len())?]
    .clone())
}

pub fn artifact_type_type(data: Arc<UnitArchipelago>) -> Result<String, Error> {
    Ok(data.lexical_unit_lagoon.artifact_type.unit_pool
        [prelude::random_index(data.lexical_unit_lagoon.artifact_type.unit_pool.len())?]
    .clone())
}

pub fn artifact_adjective_type(data: Arc<UnitArchipelago>) -> Result<String, Error> {
    Ok(data.lexical_unit_lagoon.artifact_adjective.unit_pool
        [prelude::random_index(data.lexical_unit_lagoon.artifact_adjective.unit_pool.len())?]
    .clone())
}

pub fn quality_type(data: Arc<UnitArchipelago>) -> Result<String, Error> {
    Ok(data.lexical_unit_lagoon.artifact_quality.unit_pool
        [prelude::random_index(data.lexical_unit_lagoon.artifact_quality.unit_pool.len())?]
    .clone())
}
...
pub fn general_currency_name_die(data: Arc<UnitArchipelago>) -> Result<String, Error> {
    match prelude::random_from_range(0, 2)? {
        0 => match prelude::random_from_range(0, 2)? {
...
pub fn random_currency_name_type(
    data: Arc<UnitArchipelago>,
    random_letter_amount: usize,
) -> Result<String, Error> {
    if random_letter_amount < 1 {
        Err(Error::other(format!(
            "random_letter_amount '{random_letter_amount}' less than 1!"
        )))
    } else {
...
pub fn random_abc_type(data: Arc<UnitArchipelago>) -> Result<String, Error> {
    Ok(data.lexical_unit_lagoon.abc.unit_pool
        [prelude::random_index(data.lexical_unit_lagoon.abc.unit_pool.len())?]
    .clone())
}

fn random_currency_ending_type(data: Arc<UnitArchipelago>) -> Result<String, Error> {
    Ok(data.lexical_unit_lagoon.currency_endings.unit_pool
        [prelude::random_index(data.lexical_unit_lagoon.currency_endings.unit_pool.len())?]
    .clone())
}

fn random_currency_second_word_type(data: Arc<UnitArchipelago>) -> Result<String, Error> {
    Ok(
        data.lexical_unit_lagoon.currency_second_word.unit_pool[prelude::random_index(
...
fn random_currency_real_fractional_name_type(data: Arc<UnitArchipelago>) -> Result<String, Error> {
    Ok(data
        ...
        .unit_pool[prelude::random_index(
        data.lexical_unit_lagoon
            .currency_real_fractional_names
            .unit_pool
            .len(),
...
fn random_currency_real_name_type(data: Arc<UnitArchipelago>) -> Result<String, Error> {
    Ok(data.lexical_unit_lagoon.currency_real_names.unit_pool
        [prelude::random_index(data.lexical_unit_lagoon.currency_real_names.unit_pool.len())?]
    .clone())
}

pub fn random_place_noun_type(data: Arc<UnitArchipelago>) -> Result<String, Error> {
    Ok(data.lexical_unit_lagoon.place_noun.unit_pool
        [prelude::random_index(data.lexical_unit_lagoon.place_noun.unit_pool.len())?]
    .clone())
}
