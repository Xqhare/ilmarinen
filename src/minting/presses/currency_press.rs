...
use self::currency::{
    decimal_currency_system_press, non_decimal_currency_system_press, time_currency_system_press,
};
...
pub fn currency_press(
    data: Arc<UnitArchipelago>,
    decimal_nondecimal_time_based_choice: &str,
) -> Result<String, Error> {
...
mod currency {
    ...
    use tyche::prelude::{random_from_range, random_index};
    use unicode_segmentation::UnicodeSegmentation;

    use crate::{
        minting::presses::{
            common::{
                artifact_type_type, formal_name_die, full_people_name_die,
                general_currency_name_die, random_abc_type,
            },
            metal_alloy_press::metal_alloy_press,
        },
        unit_pools::UnitArchipelago,
    };

    pub fn non_decimal_currency_system_press(
        data: Arc<UnitArchipelago>,
    ...
    pub fn decimal_currency_system_press(
        data: Arc<UnitArchipelago>,
        currency_name: String,
    ) -> Result<String, Error> {
        match random_from_range(0, 5)? {
    ...
    pub fn time_currency_system_press(
        data: Arc<UnitArchipelago>,
        currency_name: String,
    ) -> Result<String, Error> {
        match random_from_range(0, 5)? {
    ...
    pub fn decimal_currency_system_die(
        data: Arc<UnitArchipelago>,
        currency_name: String,
        unit_amount: usize,
    ) -> Result<String, Error> {
        ...
        let mut out: Vec<String> = vec![
            format!("This is a decimal currency."),
            format!("The currency is called {currency_name} and abbreviated as {abbr}."),
        ];
        // Main unit
        out.push(format!("The main unit is called {currency_name}"));
        out.push(currency_coin_denomination_and_material_die(
            data.clone(),
            &currency_name,
        )?);
            ...
            for unit in 0..unit_amount {
                if unit == 0 {
                    ...
                    if random_from_range(0, 9)? == 0 {
                        ...
                        out.insert(
                            3,
                            currency_coin_denomination_and_material_die(
                                data.clone(),
                                &super_unit_name,
                            )?,
                        );
                        out.insert(
                            4,
                            format!(
                                "The super unit {} exchanges for 1 {} = 100 {}.",
                                super_unit_name,
                                super_unit_name,
                                currency_name.clone()
                            ),
                    ...
                    let sub_unit_name = general_currency_name_die(data.clone())?;
                    out.push(format!(
                        "The {actual_number}. sub-unit is called {sub_unit_name}."
                    ));
                    out.push(currency_coin_denomination_and_material_die(
                        data.clone(),
                        &sub_unit_name,
    ...
    pub fn non_decimal_currency_system_die(
        data: Arc<UnitArchipelago>,
        currency_name: String,
        unit_amount: usize,
    ) -> Result<String, Error> {
        let abbr = currency_abbreviation_type(currency_name.clone())?;
        let mut out: Vec<String> = vec![
            format!("This is a non decimal currency."),
            format!("The currency is called {currency_name} and abbreviated as {abbr}."),
        ];
        // Main unit
        out.push(format!("The main unit is called {currency_name}"));
                ...
                .len()
        {
            return Err(Error::other(format!(
                "Only {} different coin values in database. Requested {} different coin values.",
                data.numerical_unit_lagoon
                    .currency_non_decimal_base
                    .unit_pool
                    .len(),
                unit_amount
                ...
                if unit == 0 {
                    ...
                    if random_from_range(0, 9)? == 0 {
                        ...
                        out.insert(
                            4,
                            format!(
                                "The super unit {} exchanges for 1 {} = {} {}.",
                                super_unit_name,
                                super_unit_name,
                                super_base,
                                currency_name.clone()
    ...
    pub fn time_currency_system_die(
        data: Arc<UnitArchipelago>,
        currency_name: String,
        unit_amount: usize,
    ) -> Result<String, Error> {
        let abbr = currency_abbreviation_type(currency_name.clone())?;
        let mut out: Vec<String> = vec![
            format!("This is a time based currency."),
            format!("The currency is called {currency_name} and abbreviated as {abbr}."),
        ];
        // Main unit
    ...
    pub fn currency_coin_denomination_and_material_die(
        data: Arc<UnitArchipelago>,
        unit_name: &str,
    ) -> Result<String, Error> {
        ...
        let mut out: Vec<String> = vec![format!(
            "The {} has the 1 coin. The coin {}. {}.",
            unit_name,
            currency_material_type(data.clone())?,
            currency_coin_iconography_press(data.clone())?
        )];
        for denomination in denominations {
            out.push(format!(
                "The {denomination} coin, {}. {}.",
                currency_material_type(data.clone())?,
                currency_coin_iconography_press(data.clone())?
            ));
        }
    ...
    pub fn currency_coin_iconography_press(data: Arc<UnitArchipelago>) -> Result<String, Error> {
        let out = [
            format!(
                "It shows {} on it's front,",
                currency_coin_iconography_die(data.clone(), false)?
            ),
            format!(
                "and it shows {} on it's back",
                currency_coin_iconography_die(data, true)?
            ),
        ];
        Ok(out.join("\n"))
    }

    pub fn currency_coin_iconography_die(
        data: Arc<UnitArchipelago>,
        reverse: bool,
    ) -> Result<String, Error> {
        let seed = {
        ...
        match random_from_range(0, seed)? {
            0 => currency_icon_inscription_type(data),
            1 => currency_icon_figure_type(data),
            2 => currency_icon_partial_die(data),
            3 => Ok(format!(
                "a incuse of a {}",
                currency_icon_incuse_type(data)?
            )),
            _ => Ok("the same motif repeated".to_string()),
        }
    }
    ...
    pub fn currency_icon_figure_type(data: Arc<UnitArchipelago>) -> Result<String, Error> {
        ...
        match type_to_check.as_str() {
            ...
            "Creature" => match random_from_range(0, 4)? {
                0 => Ok(format!(
                    "a mythical {} and a {}",
                    currency_icon_mythical_creature_type(data.clone())?,
                    currency_coin_bonus_iconography_type(data)?
                )),
                _ => Ok(format!(
                    "a mythical {}",
                    currency_icon_mythical_creature_type(data)?
                )),
            },
            "Animal" => match random_from_range(0, 4)? {
                0 => Ok(format!(
                    "a {} and a {}",
                    currency_icon_animal_type(data.clone())?,
                    currency_coin_bonus_iconography_type(data)?
                )),
                _ => Ok(format!("a {}", currency_icon_animal_type(data)?)),
            },
            "Plant" => match random_from_range(0, 4)? {
                0 => Ok(format!(
                    "a {} and a {}",
                    currency_icon_plant_type(data.clone())?,
                    currency_coin_bonus_iconography_type(data)?
                )),
                _ => Ok(format!("a {}", currency_icon_plant_type(data)?)),
            },
            "Object" => match random_from_range(0, 4)? {
                0 => Ok(format!("a {}", currency_figure_type(data)?)),
                _ => Ok(format!("a {}", artifact_type_type(data)?)),
            },
            _ => Err(Error::other(format!(
                "{type_to_check} from json library is not implemented in icon_figure_type!"
            ))),
        }
    }
    ...
    pub fn currency_icon_partial_die(data: Arc<UnitArchipelago>) -> Result<String, Error> {
        match random_from_range(0, 4)? {
            0 => Ok(format!(
                "a {} and a {}",
                currency_icon_partial_type(data.clone())?,
                currency_coin_bonus_iconography_type(data)?
            )),
            _ => Ok(format!("a {}", currency_icon_partial_type(data)?)),
        }
    }

    pub fn currency_coin_bonus_iconography_type(
        data: Arc<UnitArchipelago>,
    ) -> Result<String, Error> {
        let insignia = {
    ...
    pub fn currency_icon_inscription_type(data: Arc<UnitArchipelago>) -> Result<String, Error> {
        ...
        match type_to_check.as_str() {
            "Letter" => Ok(format!(
                "the letter {}",
                random_abc_type(data)?.to_uppercase()
            )),
            "Monogram" => Ok(format!(
                "a monogram of the letters {} and {}",
                random_abc_type(data.clone())?.to_uppercase(),
                random_abc_type(data)?.to_uppercase()
            )),
            "Name" => {
                if random_from_range(0, 1)? == 0 {
            ...
            _ => Err(Error::other(format!(
                "{type_to_check} from json library is not implemented in icon_inscription_type!"
            ))),
        }
    }
    ...
    pub fn currency_material_type(data: Arc<UnitArchipelago>) -> Result<String, Error> {
        if random_from_range(0, 1)? == 0 {
            // mono metallic coin
            Ok(format!("is made of {}", metal_alloy_press(data.clone())?))
        } else {
            // bi metallic coin
            Ok(format!(
                "is made with a {} center plug and a {} outer ring",
                metal_alloy_press(data.clone())?,
                metal_alloy_press(data.clone())?
            ))
        }
    }
    ...
    pub fn currency_abbreviation_type(currency_name: String) -> Result<String, Error> {
        if random_from_range(0, 1)? == 0 {
            if currency_name.len() >= 3 {
                let mut tmp_name_bind =
                    UnicodeSegmentation::graphemes(currency_name.as_str(), true)
                        .collect::<Vec<&str>>();
                ...
                Err(Error::other(format!(
                    "Name ({currency_name}) is shorter than 3 letters!"
                )))
            }
        } else {
            if currency_name.len() >= 3 {
    ...
    pub fn currency_time_fractions_type(data: Arc<UnitArchipelago>) -> Result<String, Error> {
        Ok(
            data.numerical_unit_lagoon.currency_time_fractions.unit_pool[random_index(
                data.numerical_unit_lagoon
                    .currency_time_fractions
                    .unit_pool
                    .len(),
    ...
    pub fn currency_inscription_type(data: Arc<UnitArchipelago>) -> Result<String, Error> {
        Ok(
            data.lexical_unit_lagoon.currency_icon_inscription.unit_pool[random_index(
    ...
    pub fn currency_figure_type(data: Arc<UnitArchipelago>) -> Result<String, Error> {
        Ok(
            data.lexical_unit_lagoon.currency_icon_figure.unit_pool[random_index(
                data.lexical_unit_lagoon
    ...
    pub fn currency_person_social_gender_type(data: Arc<UnitArchipelago>) -> Result<String, Error> {
        Ok(
            data.lexical_unit_lagoon.person_social_gender.unit_pool[random_index(
                data.lexical_unit_lagoon
    ...
    pub fn currency_icon_insignia_type(data: Arc<UnitArchipelago>) -> Result<String, Error> {
        Ok(
            data.lexical_unit_lagoon.currency_icon_insignia.unit_pool[random_index(
                data.lexical_unit_lagoon
    ...
    pub fn currency_icon_mythical_creature_type(
        data: Arc<UnitArchipelago>,
    ) -> Result<String, Error> {
        Ok(data
            .lexical_unit_lagoon
    ...
    pub fn currency_icon_animal_type(data: Arc<UnitArchipelago>) -> Result<String, Error> {
        Ok(
            data.lexical_unit_lagoon.currency_icon_animal.unit_pool[random_index(
                data.lexical_unit_lagoon
    ...
    pub fn currency_icon_plant_type(data: Arc<UnitArchipelago>) -> Result<String, Error> {
        Ok(data.lexical_unit_lagoon.currency_icon_plant.unit_pool
            [random_index(data.lexical_unit_lagoon.currency_icon_plant.unit_pool.len())?]
        .clone())
    }

    pub fn currency_icon_partial_type(data: Arc<UnitArchipelago>) -> Result<String, Error> {
        Ok(
            data.lexical_unit_lagoon.currency_icon_partial.unit_pool[random_index(
                data.lexical_unit_lagoon
    ...
    pub fn currency_icon_incuse_type(data: Arc<UnitArchipelago>) -> Result<String, Error> {
        Ok(
            data.lexical_unit_lagoon.currency_icon_incuse.unit_pool[random_index(
                data.lexical_unit_lagoon
